...
if args.stage == 'pivtrain':
    from dataloader import pivlist_train as lp
elif args.stage == 'pivtrainval':
    from dataloader import pivlist as lp
...
if args.stage == 'chairs':
    ...
    data_inuse = torch.utils.data.ConcatDataset(
        [loader_kitti15] * 200 + [loader_hd1k] * 40 + [loader_sintel] * 150 + [loader_chairs] * 2 + [loader_things])
    for i in data_inuse.datasets:
        i.black = True
        i.cover = True
elif 'piv' in args.stage:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (imgL_crop, imgR_crop, flowl0) in enumerate(TrainImgLoader):
            ...
            if total_iters % 100 == 0:
                log.image_summary('train/left', imgL_crop[0:1]+torch.from_numpy(np.asarray(mean_L).mean(0)[np.newaxis, :, np.newaxis, np.newaxis]), total_iters)
                log.image_summary('train/right', imgR_crop[0:1]+torch.from_numpy(np.asarray(mean_R).mean(0)[np.newaxis, :, np.newaxis, np.newaxis]), total_iters)
