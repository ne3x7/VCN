...
def test_compressibility(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        sns.kdeplot(c_true.flatten(), c="b", label="true", ax=ax, alpha=0.3)
        sns.kdeplot(c_pred.flatten(), c="y", label="pred", ax=ax, alpha=0.3)
...
def test_energy_spectrum(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.loglog(xvals_true, yvals_true, c="b", label="true", alpha=0.3)
        ax.loglog(xvals_pred, yvals_pred, c="y", label="pred", alpha=0.3)
...
def test_intermittency_r(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.loglog(r_vals, vals_true, label="true", c="b", alpha=0.3)
        ax.loglog(r_vals, vals_pred, label="pred", c="y", alpha=0.3)
...
def test_intermittency_n(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.semilogy(n_vals, vals_true, c="b", label="true", alpha=0.3)
        ax.semilogy(n_vals, vals_pred, c="y", label="pred", alpha=0.3)
        interms_true.append(vals_true)
        interms_pred.append(vals_pred)
