...
def test_compressibility(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        sns.kdeplot(c_true.flatten(), label="true", ax=ax, alpha=0.1)
        sns.kdeplot(c_pred.flatten(), label="pred", ax=ax, alpha=0.1)
...
def test_energy_spectrum(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.loglog(xvals_true, yvals_true, label="true", alpha=0.1)
        ax.loglog(xvals_pred, yvals_pred, label="pred", alpha=0.1)
        pspecs_pred.append(yvals_pred)
        pspecs_true.append(yvals_true)
    ax.loglog(xvals_true, np.mean(pspecs_true, axis=0), label="mean true", alpha=0.5)
    ax.loglog(xvals_pred, np.mean(pspecs_pred, axis=0), label="mean pred", alpha=0.5)
...
def test_intermittency_r(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.loglog(r_vals, vals_true, label="true", alpha=0.1)
        ax.loglog(r_vals, vals_pred, label="pred", alpha=0.1)
        interms_true.append(vals_true)
        interms_pred.append(vals_pred)
    ax.loglog(r_vals, np.mean(interms_true, axis=0), label="mean true", alpha=0.5)
    ax.loglog(r_vals, np.mean(interms_pred, axis=0), label="mean pred", alpha=0.5)
...
def test_intermittency_n(pairs, fname):
    ...
    for v_true, v_pred in pairs:
        ...
        ax.semilogy(n_vals, vals_true, label="true", alpha=0.1)
        ax.semilogy(n_vals, vals_pred, label="pred", alpha=0.1)
        interms_true.append(vals_true)
        interms_pred.append(vals_pred)
    ax.semilogy(n_vals, np.mean(interms_true, axis=0), label="mean true", alpha=0.5)
    ax.semilogy(n_vals, np.mean(interms_pred, axis=0), label="mean pred", alpha=0.5)
