...
class VCN(nn.Module):
    ...
    def __init__(self, size, md=[4, 4, 4, 4, 4], fac=1.):
        ...
        if size[0] > 1:  # only in train mode
            ...
            self.dc5_convo = nn.Sequential(
                conv(128 + 2 * 4 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1),
                conv(128, 128, kernel_size=3, stride=1, padding=2, dilation=2),
                conv(128, 128, kernel_size=3, stride=1, padding=4, dilation=4),
                conv(128, 96, kernel_size=3, stride=1, padding=8, dilation=8),
                conv(96, 64, kernel_size=3, stride=1, padding=16, dilation=16),
            ...
            self.dc4_convo = nn.Sequential(
                conv(128 + 3 * 4 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1),
                conv(128, 128, kernel_size=3, stride=1, padding=2, dilation=2),
                conv(128, 128, kernel_size=3, stride=1, padding=4, dilation=4),
                conv(128, 96, kernel_size=3, stride=1, padding=8, dilation=8),
                conv(96, 64, kernel_size=3, stride=1, padding=16, dilation=16),
            ...
            self.dc1_convo = nn.Sequential(
                conv(64 + 16 * fdimb1 + 8 * fdimb2, 128, kernel_size=3, stride=1, padding=1, dilation=1),
                conv(128, 128, kernel_size=3, stride=1, padding=2, dilation=2),
                conv(128, 128, kernel_size=3, stride=1, padding=4, dilation=4),
                conv(128, 96, kernel_size=3, stride=1, padding=8, dilation=8),
                conv(96, 64, kernel_size=3, stride=1, padding=16, dilation=16),
    ...
    def corr(self, refimg_fea, targetimg_fea, maxdisp, fac=1):
        ...
        corr = SpatialCorrelationSampler(kernel_size=1, patch_size=(int(1 + 2 * maxdisp // fac), int(1 + 2 * maxdisp)),
    ...
    def forward(self, im, disc_aux=None):
        ...
        flow5h = torch.cat(
            (flow5h, F.upsample(flow6h.detach() * 2, [flow5h.shape[2], flow5h.shape[3]], mode='bilinear')),
            1)  # b, k2--k2, h, w
        ...
        # append coarse hypotheses
        flow4h = torch.cat(
            (flow4h, F.upsample(flow5h.detach() * 2, [flow4h.shape[2], flow4h.shape[3]], mode='bilinear')), 1)
        ...
        # append coarse hypotheses
        flow3h = torch.cat(
            (flow3h, F.upsample(flow4h.detach() * 2, [flow3h.shape[2], flow3h.shape[3]], mode='bilinear')), 1)
        ...
        # append coarse hypotheses
        flow2h = torch.cat(
            (flow2h, F.upsample(flow3h.detach() * 2, [flow2h.shape[2], flow2h.shape[3]], mode='bilinear')), 1)
